...
def rating_impact(cat, current_reviews):
    affected=[r for r in current_reviews if any(a["category"]==cat for a in r["aspects"])]
    unaffected=[r for r in current_reviews if not any(a["category"]==cat for a in r["aspects"])]
    if not affected or not unaffected:
        return {"affected_reviews":len(affected),"affected_avg_rating":round(sum(r["rating"] for r in affected)/len(affected),2) if affected else 0,
                "unaffected_avg_rating":round(sum(r["rating"] for r in unaffected)/len(unaffected),2) if unaffected else 0,"rating_gap":0}
    aa=sum(r["rating"] for r in affected)/len(affected); ua=sum(r["rating"] for r in unaffected)/len(unaffected)
    return {"affected_reviews":len(affected),"affected_avg_rating":round(aa,2),"unaffected_avg_rating":round(ua,2),"rating_gap":round(aa-ua,2)}

def confidence_label(n):
    return "High" if n>=10 else ("Medium" if n>=5 else "Early signal")

def negative_pair_patterns(items):
    counts=Counter(); ratings=defaultdict(list); ids=defaultdict(list)
    for r in items:
        cats=sorted({a["category"] for a in r["aspects"] if a["sentiment"]=="Negative"})
        for i in range(len(cats)):
            for j in range(i+1,len(cats)):
                pair=(cats[i],cats[j]); counts[pair]+=1; ratings[pair].append(r["rating"]); ids[pair].append(r["review_id"])
    out=[]
    for pair,n in counts.most_common():
        if n<2: continue
        out.append({"categories":list(pair),"reviews":n,
                    "average_rating":round(sum(ratings[pair])/len(ratings[pair]),2),
                    "review_ids":ids[pair][:10],"confidence":confidence_label(n)})
    return out[:8]

def positive_negative_patterns(items):
    counts=Counter(); ratings=defaultdict(list); ids=defaultdict(list)
    for r in items:
        pos=sorted({a["category"] for a in r["aspects"] if a["sentiment"]=="Positive"})
        neg=sorted({a["category"] for a in r["aspects"] if a["sentiment"]=="Negative"})
        for a in pos:
            for b in neg:
                if a==b: continue
                pair=(a,b); counts[pair]+=1; ratings[pair].append(r["rating"]); ids[pair].append(r["review_id"])
    out=[]
    for (positive,negative),n in counts.most_common():
        if n<2: continue
        out.append({"positive":positive,"negative":negative,"reviews":n,
                    "average_rating":round(sum(ratings[(positive,negative)])/len(ratings[(positive,negative)]),2),
                    "review_ids":ids[(positive,negative)][:10],"confidence":confidence_label(n),
                    "title":f"Customers like {positive.lower()}, but struggle with {negative.lower()}"})
    return out[:6]

def menu_issue_links(items):
    stats={}
    for r in items:
        neg={a["category"] for a in r["aspects"] if a["sentiment"]=="Negative"}
        for dish in r.get("menu_items",[]):
            key=stats.setdefault(dish,{"mentions":0,"ratings":[],"issues":Counter(),"review_ids":[]})
            key["mentions"]+=1; key["ratings"].append(r["rating"]); key["review_ids"].append(r["review_id"])
            key["issues"].update(neg)
    out=[]
    for dish,x in stats.items():
        if not x["mentions"]: continue
        top=x["issues"].most_common(3)
        out.append({"item":dish,"mentions":x["mentions"],
                    "average_rating":round(sum(x["ratings"])/len(x["ratings"]),2),
                    "top_negative_issues":[{"category":c,"reviews":n} for c,n in top],
                    "review_ids":x["review_ids"][:10],
                    "confidence":confidence_label(x["mentions"])})
    return sorted(out,key=lambda x:(sum(i["reviews"] for i in x["top_negative_issues"]),x["mentions"]),reverse=True)[:10]

def health_score(current_reviews, category_stats, alerts):
    if not current_reviews: return 0
    avg=sum(r["rating"] for r in current_reviews)/len(current_reviews)
    rating_component=(avg/5)*60
    neg_rate=sum(r["sentiment"]=="Negative" for r in current_reviews)/len(current_reviews)
    sentiment_component=(1-neg_rate)*25
    priority_penalty=(max([x.get("priority_score",0) for x in category_stats] or [0])/100)*10
    alert_penalty=min(5,len(alerts)*1.5)
    return round(max(0,min(100,rating_component+sentiment_component+15-priority_penalty-alert_penalty)))

def operational_recommendation(cat, pair=None):
    if pair:
        a,b=pair
        special={
            frozenset(("Price / Value","Portion Size")):"Audit portion consistency and price-to-portion perception together; customers may be describing one value problem in two ways.",
            frozenset(("Service","Wait Time")):"Separate front-of-house delay from kitchen delay. Review peak-period staffing, table touch frequency, ticket times and handoffs.",
            frozenset(("Food Quality","Delivery / Takeout")):"Check packaging, holding time and pickup/delivery handoff; food quality may be degrading after it leaves the kitchen.",
            frozenset(("Order Accuracy","Delivery / Takeout")):"Add a final bag/order verification step before pickup and delivery handoff.",
            frozenset(("Cleanliness","Service")):"Assign explicit front-of-house ownership for table resets and cleanliness checks during busy periods."
        }
        if frozenset((a,b)) in special: return special[frozenset((a,b))]
    return ACTION_MAP.get(cat,f"Investigate recurring {cat.lower()} feedback and verify the operational cause.")

def build_operational_intelligence(cur, category_stats, alerts, menu_stats):
    bycat={x["category"]:x for x in category_stats}
    priorities=[]
    for x in sorted([z for z in category_stats if z["negative"]],key=lambda z:(z["priority_score"],z["negative"]),reverse=True)[:5]:
        impact=rating_impact(x["category"],cur)
        priorities.append({
            "category":x["category"],"score":x["priority_score"],
            "severity":"High" if x["priority_score"]>=65 else ("Medium" if x["priority_score"]>=40 else "Low"),
            "negative_mentions":x["negative"],"mentions":x["mentions"],
            "negative_rate":round(x["negative_rate"]*100,1),"negative_change":x["negative_change"],
            "affected_avg_rating":impact["affected_avg_rating"],"unaffected_avg_rating":impact["unaffected_avg_rating"],
            "rating_gap":impact["rating_gap"],"confidence":confidence_label(x["mentions"]),
            "why_it_matters":f'Reviews mentioning {x["category"].lower()} average {impact["affected_avg_rating"]}★'
                + (f', {abs(impact["rating_gap"])} stars lower than reviews without it.' if impact["rating_gap"]<0 else '.'),
            "action":operational_recommendation(x["category"]),
            "evidence":representative_evidence(x["category"],cur,3)
        })
    negpairs=negative_pair_patterns(cur)
    root_causes=[]
    for p in negpairs[:5]:
        a,b=p["categories"]
        root_causes.append({
            "title":f"{a} + {b}",
            "reviews":p["reviews"],"average_rating":p["average_rating"],"confidence":p["confidence"],
            "insight":f'{p["reviews"]} reviews complain about both {a.lower()} and {b.lower()}; those reviews average {p["average_rating"]}★.',
            "action":operational_recommendation(a,(a,b)),"review_ids":p["review_ids"]
        })
    tradeoffs=positive_negative_patterns(cur)
    menu_links=menu_issue_links(cur)
    return {
        "health_score":health_score(cur,category_stats,alerts),
        "sample_size":len(cur),
        "sample_confidence":confidence_label(len(cur)),
        "priorities":priorities,
        "root_cause_patterns":root_causes,
        "customer_tradeoffs":tradeoffs,
        "menu_issue_links":menu_links,
        "method_note":"Priority scores combine frequency, severity, negative-review share and recent change. Rating impact compares reviews mentioning an issue with reviews that do not. Patterns are hypotheses for investigation, not proof of causation."
    }
...
def run():
    ...
    operational=build_operational_intelligence(cur,category_stats,alerts,menu)
    out={"analysis_version":"2.1","generated_at":datetime.now().isoformat(timespec="seconds"),"data_mode":data_mode,"source_file":source_file,"operational_intelligence":operational,"business":{"name":settings["business_name"],"location":settings["location_name"]},"window":{"days":30,"start":cur_start.isoformat(),"end":latest.isoformat()},"metrics":{"reviews":len(cur),"average_rating":avg,"positive_pct":pct(pos,len(cur)),"negative_pct":pct(neg,len(cur)),"alerts":len(alerts)},"category_stats":category_stats,"priority_issues":priorities,"patterns":patterns,"menu_stats":menu,"monthly_trend":monthly,"alerts":alerts,"brief":brief,"response_drafts":drafts,"reviews":enriched}
