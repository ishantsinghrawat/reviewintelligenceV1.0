...
def classify_aspects(text,taxonomy,rating):
    ...
    for cat in taxonomy["categories"]:
        ...
        evidence=list(dict.fromkeys(evidence)); sent,conf=local_sentiment(" ".join(evidence),rating)
        if cat=="Wait Time":
            strong=any(re.search(p,norm(text)) for p in CATEGORY_PHRASES["Wait Time"])
            if not strong and set(norm(x) for x in hits)<={"wait","waiting","slow","late","quick","minutes"}: continue
        score=min(.98,.62+.06*min(5,len(set(hits)))+.03*min(3,len(evidence)))
        out[cat]={"category":cat,"score":round(score,3),"sentiment":sent,"sentiment_confidence":round(conf,3),"hits":sorted(set(hits))[:8],"evidence":evidence[:3]}
    if not out:
        out["Food Quality"]={"category":"Food Quality","score":.35,"sentiment":sentiment_from_rating(rating),"sentiment_confidence":.35,"hits":[],"evidence":[text] if text else []}
    return sorted(out.values(),key=lambda x:(x["score"],x["sentiment"]=="Negative"),reverse=True)
